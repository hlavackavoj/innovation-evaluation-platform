...
import { requireCurrentUser, AuthorizationError } from "@/lib/authorization";
...
async function handleRequest() {
  try {
    ...
    const details = error instanceof Prisma.PrismaClientKnownRequestError ? error.meta : undefined;
    const maybeMigrationHint =
      message.includes("Task_contactId") || message.includes("column") || message.includes("P2022")
        ? "Likely missing DB migration. Run: npm run prisma:migrate"
        : undefined;

    console.error("TEST_EMAIL_ANALYSIS_FAILED", error);

    return NextResponse.json(
      {
        error: "Test email analysis failed",
        message,
        details,
        hint: maybeMigrationHint
      },
